...
int main(int argc, char** argv)
{
  ...
  private_node_handle.param<std::string>("tf_parent_frame_id", tf_parent_frame_id, "sensor_base");
  private_node_handle.param<std::string>("tf_frame_id", tf_frame_id, "imu_link");
  private_node_handle.param<std::string>("imu_frame_id", imu_frame_id, "imu_link");
  private_node_handle.param<std::string>("mag_frame_id", mag_frame_id, "mag_link");
  ...
  ros::Publisher magnet_pub = nh.advertise<sensor_msgs::MagneticField>("mag", 50);
    ...
    try
    {
      if (ser.isOpen())
      {
        ...
        if(ser.available())
        {
          ...
          input += read;
          while (input.length() >= 34) // while there might be a complete package in input
          {
            ...
            if (data_packet_start != std::string::npos)
            {
              ...
              if ((input.length() >= data_packet_start + 34) && (input.compare(data_packet_start + 32, 2, "\r\n") == 0))  //check if positions 32,33 exist, then test values
              {
                ...
                uint8_t received_message_number = input[data_packet_start + 31];
                ...
                temperature_msg.header.frame_id = imu_frame_id;
                ...
                mag.magnetic_field.x = mxf;
                mag.magnetic_field.y = myf;
                mag.magnetic_field.z = mzf;

                magnet_pub.publish(mag);
                ...
                input.erase(0, data_packet_start + 34); // delete everything up to and including the processed packet
              }
              else
              {
                if (input.length() >= data_packet_start + 34)
                {
